    ...
    public partial class adminbookinventory : System.Web.UI.Page
    {
        ...
        void updateBookByID()
        {
            if (checkIfBookExists())
            {
                try
                {
                    ...
                    string filepath = global_filepath; // Use the current file path
                    string filename = Path.GetFileName(FileUpload1.PostedFile.FileName);

                    // If a new file is uploaded, replace the current one
        ...
        void getBookByID()
        {
            try
            {
                ...
                if (dt.Rows.Count >= 1)
                {
                    ...
                    DropDownList1.SelectedValue = dt.Rows[0]["type_of_language"].ToString().Trim();
        ...
        bool checkIfBookExists()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("SELECT * FROM book_master WHERE book_id = @book_id and book_name = @book_name COLLATE SQL_Latin1_General_CP1_CI_AS", con);
                cmd.Parameters.AddWithValue("@book_id", TextBox1.Text.Trim());
                cmd.Parameters.AddWithValue("@book_name", TextBox2.Text.Trim());
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
        ...
        void addNewBook()
        {
            try
            {
                ...
                string filepath = "~/imgs/default_book.png"; // Default image
                string filename = Path.GetFileName(FileUpload1.PostedFile.FileName);
