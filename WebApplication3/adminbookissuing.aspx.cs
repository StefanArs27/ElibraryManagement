    ...
    public partial class adminbookissuing : System.Web.UI.Page
    {
        ...
        void returnBook()
        {
            try
            {
                ...
                if (result > 0)
                {
                    ...
                    cmd = new SqlCommand("update book_master set current_stock = current_stock+1 WHERE book_id='" + TextBox1.Text.Trim() + "'", con);
                    cmd.ExecuteNonQuery();
                    con.Close();
        ...
        bool checkIfBookExist()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("select * from book_master WHERE book_id='" + TextBox1.Text.Trim() + "' AND current_stock >0", con);
        ...
        bool checkIfMemberExist()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("select full_name from member_master WHERE member_id='" + TextBox2.Text.Trim() + "'", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows.Count >= 1)
        ...
        void getNames()
        {
            try
            {
                ...
                SqlCommand cmd = new SqlCommand("select book_name from book_master WHERE book_id='" + TextBox1.Text.Trim() + "'", con);
                SqlDataAdapter da = new SqlDataAdapter(cmd);
                DataTable dt = new DataTable();
                da.Fill(dt);
                if (dt.Rows.Count >= 1)
